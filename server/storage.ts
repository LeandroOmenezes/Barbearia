...
export class MemStorage implements IStorage {
  ...
  async createProfessional(data: InsertProfessional): Promise<Professional> {
    return { id: Date.now(), ...data, bio: data.bio ?? null, photoBase64: data.photoBase64 ?? null, photoMimeType: data.photoMimeType ?? null, active: data.active ?? true, createdAt: new Date() };
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async isDateBlocked(date: string, professionalId?: number | null): Promise<{ blocked: boolean; reason?: string; description?: string }> {
    ...
    // Match global blocks (professionalId IS NULL) OR blocks for the specific professional
