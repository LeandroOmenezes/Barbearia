...
type AuthContextType = {
  ...
  forgotPasswordMutation: UseMutationResult<{ message: string; resetLink?: string; emailFailed?: boolean }, Error, { email: string }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const forgotPasswordMutation = useMutation({
    mutationFn: async ({ email }: { email: string }) => {
      ...
      return await res.json() as { message: string; resetLink?: string; emailFailed?: boolean };
    },
    onSuccess: (data) => {
      if (!data.emailFailed) {
        toast({
      ...
      // Se emailFailed, o link é exibido diretamente no formulário — sem toast
    },
    onError: (error: Error) => {
      toast({
        title: "Erro ao gerar link",
        description: "Não foi possível gerar o link de recuperação. Tente novamente.",
        variant: "destructive",
      });
    },
  });
