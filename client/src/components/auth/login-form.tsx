        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full">
            ...
            <form onSubmit={handleForgotPassword}>
              ...
              {forgotPasswordMutation.isSuccess && !forgotPasswordMutation.data?.emailFailed && (
                <div className="mt-4 p-3 bg-green-50 rounded-lg">
                  <p className="text-sm text-green-700">
              ...
              {forgotPasswordMutation.isSuccess && forgotPasswordMutation.data?.emailFailed && forgotPasswordMutation.data?.resetLink && (
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <p className="text-sm text-amber-800 mb-2">
                    <strong>⚠️ Email indisponível — use o link abaixo:</strong>
                  </p>
                  <a
                    href={forgotPasswordMutation.data.resetLink}
                    className="block text-xs text-blue-700 underline break-all font-mono bg-white p-2 rounded border border-blue-200"
                  >
                    {forgotPasswordMutation.data.resetLink}
                  </a>
                  <p className="text-xs text-amber-600 mt-2">
                    Clique no link acima ou copie-o. Ele expira em 1 hora.
                  </p>
                </div>
              )}
              ...
              {forgotPasswordMutation.isError && (
                <div className="mt-4 p-3 bg-yellow-50 rounded-lg">
                  <p className="text-sm text-yellow-700">
                    <strong>⚠️ Erro ao gerar o link</strong><br />
                    <span className="text-xs">Tente novamente mais tarde.</span>
                  </p>
                </div>
              )}
            </form>
